import { MoonRes, moonTarget } from '../../../common/api';
...
let moonData: MoonRes;

async function getMoonData(): Promise<MoonRes> {
    try {
        const response = await fetch(moonTarget.resolve({}));
        const data = await response.json() as MoonRes;
